...
class Augmenter(object):
    ...
    def __init__(self, index_fn, valid_spans=None):
        ...
        for span_type in self.index["span"]:
            ...
            span_freqs_tmp = [
                self.index["span"][span_type][sp]["document_freq"]
                for sp in self.all_spans[span_type]
            ]
            self.span_freqs[span_type] = np.array(span_freqs_tmp) / np.sum(
                span_freqs_tmp
            )
    ...
    def augment(self, tokens, labels, op="token_del_tfidf"):
        ...
        if "span" in flags:
            ...
            if "sim" in op:
                ...
                new_span = np.random.choice(
                    candidates, 1, p=self.span_freqs[span_type]
                )[0]
            else:
                candidates = self.all_spans[span_type]
            ...
            if "del" in op:
                ...
                ins_token = self.sample_token(tokens[pos1], same_length=False)
                if (
                    tokens[pos1] in self.index["token"]
                    and ins_token in self.index["token"]
                ):
                    len1 = self.index["token"][tokens[pos1]]["bert_length"]
                    len2 = self.index["token"][ins_token]["bert_length"]
                    if len1 < len2:
                        # truncate the new sequence
                        bert_tokens = self.index["token"][ins_token]["bert_token"][
                            :len1
                        ]
    ...
    def augment_sent(self, text, op="token_del_tfidf"):
        ...
        for idx, span in enumerate(target_spans):
            ...
            for tid in range(len(tokens)):
                if (
                    tid + len(span_tokens) <= len(tokens)
                    and tokens[tid : tid + len(span_tokens)] == span_tokens
                ):
    ...
    def sample_position(self, tokens, labels, tfidf=False):
        ...
        for idx, token in enumerate(tokens):
            if (
                labels[idx] == "O"
                and token in index
                and index[token]["similar_words"] != None
                and len(index[token]["similar_words"]) > 0
            ):
    ...
    def sample_token(self, token, same_length=True, max_candidates=10):
        ...
        if token in index and index[token]["similar_words"] != None:
            if same_length:
                ...
                candidates = []
                for ts, bl in zip(
                    index[token]["similar_words"], index[token]["similar_words_length"]
                ):
                    t, _ = ts
                    if bl == bert_length:
                        candidates.append(t)
                ...
                candidates = [
                    t for t, _ in index[token]["similar_words"][:max_candidates]
                ]
            if len(candidates) <= 0:
                return token
            else:
