...
if __name__ == "__main__":
    ...
    # DACon variants:
    # 1. dacon_baseline: supervised (x_random_aug, x) pair with equal probability for each DA op
    #    * Loss: CE(x, y_true) + CE(x_random_aug, y_true)
    # 2. dacon_one_to_many: supervised (x_aug_1, ..., x_aug_n, x) with equal probability for each DA op and each x_aug
    #                       is generated by different DA op
    #    * Loss: CE(x, y_true) + CE(x_aug_1, y_true) + ... + CE(x_aug_n, y_true)
    # 3. dacon_fixed_consistency: supervised (x_aug_1, ..., x_aug_n, x) with equal probability for each DA op +
    #                           equal-weight JS divergence
    #    * Loss: CE(x, y_true) + CE(x_aug_1, y_true) + ... + CE(x_aug_n, y_true) + fix JS divergence
    # 4. dacon_consistency: supervised (x_aug_1, ..., x_aug_n, x) with equal probability for each DA op +
    ...
    if hp.da and "dacon" in hp.da:
        ...
        if hp.da not in [
            ...
            raise argparse.ArgumentError(
                "Incorrect argument value for dacon variants. Expected ``dacon_baseline'', ``dacon_one_to_many'', ``dacon_fixed_consistency'', ``dacon_consistency''"
            )
