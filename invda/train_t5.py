...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        train_dataset = get_dataset(
            ...
            type_path=self.hparams.train_filename,
            args=self.hparams,
        )
    ...
    def val_dataloader(self):
        val_dataset = get_dataset(
            tokenizer=self.tokenizer,
            type_path=self.hparams.valid_filename,
            args=self.hparams,
        )
        return DataLoader(
...
class ParaphraseDataset(Dataset):
    ...
    def _build(self):
        ...
        for idx in tqdm(range(len(self.data))):
            ...
            input_ = ("corrupt: " + corrupted + " </s>").replace(
                "[SEP]", "***"
            )  # avoid the special token
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_filename", type=str, default="log_no_dbpedia_train_10ptg"
    )
    parser.add_argument(
        "--valid_filename", type=str, default="log_no_dbpedia_valid_10ptg"
    )
    parser.add_argument("--model_output_dir", type=str, default="t5_paraphrase_base")
    parser.add_argument("--gpu_list", type=str, default="3")
    parser.add_argument(
        "--type", type=str, default="cls"
    )  # two mode: cls or em (cleaning)
